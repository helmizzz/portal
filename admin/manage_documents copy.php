...
function buildAdminTree($conn, $parentId = NULL)
{
    $html = '<ul class="list-group list-group-flush">';
    $sql_folders = "SELECT id, name, parent_id FROM folders WHERE parent_id " . ($parentId === NULL ? "IS NULL" : "= ?") . " ORDER BY name ASC";
    $stmt_folders = $conn->prepare($sql_folders);
    if ($parentId !== NULL)
        $stmt_folders->bind_param("i", $parentId);
    $stmt_folders->execute();
    $result_folders = $stmt_folders->get_result();
    while ($row = $result_folders->fetch_assoc()) {
        $html .= '<li class="list-group-item folder-item" data-id="' . $row['id'] . '" data-name="' . htmlspecialchars($row['name']) . '" data-parent-id="' . ($row['parent_id'] ?? '') . '">';
        $html .= '<div><input type="checkbox" class="form-check-input me-2 bulk-checkbox" name="selected_ids[]" form="bulk-action-form" value="folder-' . $row['id'] . '"><i class="fas fa-folder me-2 text-warning"></i><span class="folder-name">' . htmlspecialchars($row['name']) . '</span></div>';
        $html .= buildAdminTree($conn, $row['id']);
        $html .= '</li>';
    }
    $sql_docs = "SELECT id, title, folder_id FROM documents WHERE folder_id " . ($parentId === NULL ? "IS NULL" : "= ?") . " ORDER BY title ASC";
    $stmt_docs = $conn->prepare($sql_docs);
    if ($parentId !== NULL)
        $stmt_docs->bind_param("i", $parentId);
    $stmt_docs->execute();
    $result_docs = $stmt_docs->get_result();
    while ($row = $result_docs->fetch_assoc()) {
        $html .= '<li class="list-group-item document-item" data-id="' . $row['id'] . '" data-title="' . htmlspecialchars($row['title']) . '" data-folder-id="' . ($row['folder_id'] ?? '') . '">';
        $html .= '<div><input type="checkbox" class="form-check-input me-2 bulk-checkbox" name="selected_ids[]" form="bulk-action-form" value="doc-' . $row['id'] . '"><i class="fas fa-file-pdf me-2 text-danger"></i><span class="document-name">' . htmlspecialchars($row['title']) . '</span></div></li>';
    }
    $html .= '</ul>';
    return $html;
}

function buildFolderOptions($conn, $parentId = NULL, $prefix = '', $excludeFolderId = null)
{
    $html = '';
    $sql = "SELECT id, name FROM folders WHERE parent_id " . ($parentId === NULL ? "IS NULL" : "= ?") . " ORDER BY name ASC";
    $stmt = $conn->prepare($sql);
    if ($parentId !== NULL) {
    ...
    $result = $stmt->get_result();
    while ($row = $result->fetch_assoc()) {
        if ($row['id'] == $excludeFolderId) {
            continue;
        }
        $html .= '<option value="' . $row['id'] . '">' . $prefix . htmlspecialchars($row['name']) . '</option>';
        $html .= buildFolderOptions($conn, $row['id'], $prefix . '&nbsp;&nbsp;&nbsp;»&nbsp;', $excludeFolderId);
    }
    return $html;
}
