...
if (!empty($params)) {
    // Already has WHERE a.is_active = 1, so append with AND
    $sql .= " AND ";
...
// DELETE HANDLER (Moved to hapus_doc.php for consistency)
...
<body class="<?= $theme_class ?>">
    ...
    <div class="container-fluid mt-2">
        <!-- <div
            class="d-flex flex-column flex-lg-row align-items-start align-items-lg-center mb-2 justify-content-between gap-2">
            <h6 class="mb-0"><i class="bi bi-folder2-open me-2"></i><?= $username?> - File Manager</h6>
            <h6 class="mb-0">Daftar Dokumen:
                <strong><?= !empty($dept_filter) ? htmlspecialchars($dept_filter) : 'Semua Departemen' ?></strong>
            </h6>
            <div class="d-flex flex-wrap gap-2 align-items-center">
                <button style="height: 30px; font-size: 12px;"  type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#CreateTahunModal">
                    <i class="bi bi-calendar-plus me-1"></i> Tambah Tahun
                </button>
                <button style="height: 30px; font-size: 12px;" type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#UploadFileModal">
                    <i class="bi bi-cloud-upload me-1"></i> Upload File
                </button>
                <button style="height: 30px; font-size: 12px;" type="button" class="btn btn-secondary" data-bs-toggle="modal"
                    data-bs-target="#CreateFolderModal">
                    <i class="bi bi-folder-plus me-1"></i> Tambah Folder
                </button>
                <a href="restore.php" style="height: 30px; font-size: 12px;" class="btn btn-outline-danger">
        ...
        <div class="parent">
            ...
            <div class="div1">
                ...
                <button style=""  type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#CreateTahunModal">
                    <i class="fa fa-calendar-plus"></i>
                </button>
                <button style="" type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#UploadFileModal">
                    <i class="fa fa-file-upload"></i>
                </button>
                <!-- <button style="" type="button" class="btn btn-secondary" data-bs-toggle="modal"
                    data-bs-target="#CreateFolderModal">
                    <i class="fa fa-folder-plus"></i>
                </button> -->
                </div>
                <div class="folder-list mt-2" style="flex-grow: 1; overflow-y: auto; min-height: 0;">
                    <a href="?nama_dept=" class="folder-item <?= empty($dept_filter) ? 'active' : '' ?>">
                        <i class="fas fa-folder"></i>
                        <span style="font-size: 12px;" class="folder-title">Semua Departemen</span>
                    </a>
                    <?php foreach ($departements as $d): ?>
                        <a href="?nama_dept=<?= urlencode($d['name']) ?>&tahun=<?= urlencode($tahun_filter) ?>"
                            class="folder-item <?= ($dept_filter == $d['name']) ? 'active' : '' ?>">
                            <i class="far fa-folder"></i>
                            <span style="font-size: 12px;" class="folder-title"><?= htmlspecialchars($d['name']) ?></span>
                        </a>
                    <?php endforeach; ?>
                </div>
            </div>
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <form method="GET" class="d-flex gap-2">
                            ...
                            <select name="tahun" class="form-select form-select-sm" onchange="this.form.submit()">
                                ...
                                <?php
                                mysqli_data_seek($tahunList, 0); // Reset pointer
                            ...
                            <select name="jenis_dokumen" class="form-select form-select-sm" onchange="this.form.submit()">
                                <option value="SOP&IK">JENIS</option>
                                <option value="SOP">SOP</option>
                                <option value="IK">IK</option>
                            </select>
                            <input type="text" id="customSearch" class="form-control form-control-sm" placeholder="Cari..." style="width: 200px;">
                        </form>
                    </div>
                    <div class="table-responsive" style="flex-grow: 1; overflow-y: auto;">
                        <table id="datatable" style="width: 100%; font-size: 12px;"
                            ...
                            <tbody>
                                ...
                                function getFileIcon($filename) {
                                    ...
                                    switch ($ext) {
                                        case 'pdf': return ['icon' => 'bi-file-earmark-pdf-fill', 'color' => 'text-danger', 'type' => 'PDF Document'];
                                        case 'doc': 
                                        case 'docx': return ['icon' => 'bi-file-earmark-word-fill', 'color' => 'text-primary', 'type' => 'Microsoft Word'];
                                        case 'xls': 
                                        case 'xlsx': return ['icon' => 'bi-file-earmark-excel-fill', 'color' => 'text-success', 'type' => 'Microsoft Excel'];
                                        case 'ppt': 
                                        case 'pptx': return ['icon' => 'bi-file-earmark-ppt-fill', 'color' => 'text-warning', 'type' => 'Microsoft PowerPoint'];
                                        case 'jpg': 
                                        case 'jpeg': 
                                        case 'png': return ['icon' => 'bi-file-earmark-image-fill', 'color' => 'text-info', 'type' => 'Image File'];
                                        default: return ['icon' => 'bi-file-earmark-fill', 'color' => 'text-secondary', 'type' => strtoupper($ext) . ' File'];
                                    }
                                }

                                while ($row = mysqli_fetch_assoc($result)): 
                                    $filePath = $row['file_path'];
                                    $fileSize = file_exists($filePath) ? formatSizeUnits(filesize($filePath)) : 'Unknown';
                                    $fileInfo = getFileIcon($row['file_name']);
                                    // Use uploaded_at as is from DB
                                    $uploadDate = date('d M Y H:i', strtotime($row['uploaded_at']));
                                ?>
                                    <tr class="file-row" 
                                        data-id="<?= $row['id'] ?>"
                                        ...
                                        data-dept="<?= htmlspecialchars($row['nama_dept']) ?>"
                                        data-uploaded="<?= $uploadDate ?>"
                                        data-size="<?= $fileSize ?>"
                                        data-type="<?= $fileInfo['type'] ?>"
                                        data-icon="<?= $fileInfo['icon'] ?>"
                                        data-color="<?= $fileInfo['color'] ?>"
                                        style="cursor: pointer; border-bottom: 1px solid #f0f0f0;">
                                        
                                        <td class="text-secondary fw-bold small">
                                            <?= htmlspecialchars($row['file_code']) ?>
                                        </td>
                                        <td>
                                            <div class="d-flex align-items-center">
                                                <i class="bi <?= $fileInfo['icon'] ?> <?= $fileInfo['color'] ?> me-3 fs-5"></i>
                                                <span class="text-dark"><?= htmlspecialchars($row['file_name']) ?></span>
                                            </div>
                                        </td>
                                    </tr>
                                <?php endwhile; ?>
            ...
            <div class="div3">
                <div class="card border-0 shadow-sm p-0 h-100 w-100 overflow-hidden">
                    <div class="d-flex flex-column h-100" style="overflow-y: auto;">
                        <!-- Details Section -->
                        <div class="p-2 bg-white">
                            <div id="detail-content">
                                <?php if (!empty($dept_filter)): ?>
                                    <ul class="list-group list-group-flush small" id="inspector-list">
                                        <li class="list-group-item d-flex justify-content-between align-items-center bg-transparent">
                                            <span><i class="fas fa-folder-open text-warning"></i><span class="badge bg-primary rounded-pill"><?= htmlspecialchars($dept_filter) ?></span></span>
                                            <span>Tahun: <?= !empty($tahun_filter) ? htmlspecialchars($tahun_filter) : 'Semua Tahun' ?></span>
                                            <span><i class="bi bi-files me-2"></i> Jumlah File : <span class="badge bg-primary rounded-pill"><?= mysqli_num_rows($result) ?></span></span>
                                        </li>
                                    </ul>
                                <?php else: ?>
                                    <div class="text-center py-3 text-muted">
                                        <i class="bi bi-arrow-left-circle mb-2" style="font-size: 1.5rem;"></i>
                                        <p class="small mb-0">Pilih file untuk melihat detail</p>
                                    </div>
                                <?php endif; ?>
                            </div>
                        </div>
                        ...
                        <!-- Preview Section -->
                        <div class="flex-grow-1 bg-light position-relative p-0 d-flex flex-column">
                             <div class="flex-grow-1 position-relative" style="min-height: 200px;">
                                <div id="preview-empty" class="position-absolute top-50 start-50 translate-middle text-center text-muted">
                                    <i class="bi bi-file-earmark-pdf mb-2" style="font-size: 2rem;"></i>
                                    <p class="small">Klik file untuk preview</p>
                                </div>
                                <iframe id="sidebarPreviewIframe" src="" style="width:100%; height:100%; border:none; display:none;" allowfullscreen></iframe>
                             </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="modal-body">
                    <form id="upload" action="upload_documents.php" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-4">
                            <label class="" for="jenis_dokumen">Jenis Dokumen</label>
                ...
                <div class="modal-body">
                    <form id="editForm" action="update_documents.php" method="POST" enctype="multipart/form-data">
                        ...
                        <!-- <div class="mb-3">
                            ...
                            <select name="departements_access[]" id="edit_departements_access" class="form-select select2" multiple="multiple">
                                <?php foreach ($departements as $d): ?>
                                    <option value="<?= $d['id'] ?>"><?= htmlspecialchars($d['name']) ?></option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="users_access[]" id="edit_users_access" class="form-select select2" multiple="multiple">
                                <?php foreach ($users_list as $u): ?>
                                    <option value="<?= $u['id'] ?>"><?= htmlspecialchars($u['username']) ?></option>
                                <?php endforeach; ?>
                            </select>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(document).on('contextmenu', '.file-row', function (event) {
                ...
                menuHtml += `<li><a class="dropdown-item text-danger" href="#" data-action="delete-document" data-id="${id}" data-file="${title}" data-code="${code}"><i class="fas fa-trash me-2"></i>Delete</a></li>`;
                menuHtml += '</ul>';
            ...
            $(document).on('click', '.context-menu a', function (e) {
                ...
                if (action === 'edit-document') {
                    ...
                    if (confirm(`Apakah Anda yakin ingin menghapus dokumen "${$(this).data('code')}"?\n\nFile: ${title}\n\nTindakan ini akan memindahkan dokumen ke Recycle Bin.`)) {
                        const form = $('<form method="POST" action="hapus_doc.php">' +
                            '<input type="hidden" name="action" value="hapus_doc">' +
