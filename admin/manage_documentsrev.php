    ...
    <div class="container-fluid mt-4">
        <div
            ...
            <div class="d-flex flex-wrap gap-2">
                <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#CreateTahunModal">
                    Tambah Tahun
                </button>
            ...
            <table id="datatable" style="width: 100%;font-size: 12px;" class="table table-striped table-bordered">
                ...
                <tbody>
                    <?php while ($row = mysqli_fetch_assoc($result)): ?>
                        <tr>
                            <td><a class='view-pdf-btn' data-file='<?= htmlspecialchars($row['file_name']) ?>'
                                    data-tahun='<?= htmlspecialchars($row['tahun']) ?>'
                                    data-dept='<?= htmlspecialchars($row['nama_dept']) ?>'><i
                                        class='fa-solid fa-eye'></i></a>
                                <a class='edit-doc-btn' href='#' data-id='<?= $row['id'] ?>'
                                    data-file_code='<?= htmlspecialchars($row['file_code']) ?>'
                                    data-file_name='<?= htmlspecialchars($row['file_name']) ?>'
                                    data-tahun='<?= htmlspecialchars($row['tahun']) ?>'
                                    data-nama_dept='<?= htmlspecialchars($row['nama_dept']) ?>'
                                    data-file_path='<?= htmlspecialchars($row['file_path']) ?>'
                                    data-file_type='<?= htmlspecialchars($row['file_type']) ?>'
                                    data-file_size='<?= htmlspecialchars($row['file_size']) ?>'
                                    data-uploaded_at='<?= htmlspecialchars($row['uploaded_at']) ?>'
                                    data-updated_at='<?= htmlspecialchars($row['updated_at']) ?>'
                                    data-created_by='<?= htmlspecialchars($row['created_by']) ?>'><i
                                        class='fa-solid fa-edit'></i></a>
                                <a class='delete-doc-btn' href='#' data-id='<?= $row['id'] ?>'
                                    data-file_name='<?= htmlspecialchars($row['file_name']) ?>'
                                    data-tahun='<?= htmlspecialchars($row['tahun']) ?>'
                                    data-nama_dept='<?= htmlspecialchars($row['nama_dept']) ?>'><i
