...
<head>
    ...
    <style>
        ...
        .shortcut-link {
            ...
            display: block;
            padding: 10px;
            border-radius: .375rem; /* Sesuai radius Bootstrap */
        ...
        <div class="row flex-grow-1">
            <div class="col-md-4">
                ...
                <div class="card shadow-sm mb-4">
                    <div class="card-header">Application Shortcuts</div>
                    <div class="card-body">
                        <div class="row g-3">
                            <?php if (!empty($shortcuts)): ?>
                                <?php foreach ($shortcuts as $shortcut): ?>
                                    <div class="col-4 text-center">
                                        <a href="<?= htmlspecialchars($shortcut['url']) ?>" target="_blank" class="shortcut-link">
                                            <i class="<?= htmlspecialchars($shortcut['icon_class']) ?> fa-2x"></i>
                                            <span class="d-block"><?= htmlspecialchars($shortcut['name']) ?></span>
                                        </a>
                                    </div>
                                <?php endforeach; ?>
                            <?php else: ?>
                                <p class="text-muted text-center">No shortcuts available.</p>
                            <?php endif; ?>
                            </div>
                    </div>
                </div>                
                        ...
                        <div id="favorite-docs-list" class="list-group list-group-flush">
                           <?php if (!empty($favorite_docs)): ?>
                                <?php foreach ($favorite_docs as $doc): ?>
                                    <a href="sop.php#doc-<?= $doc['id'] ?>" class="list-group-item list-group-item-action d-flex align-items-center">
                                        <i class="fas fa-file-pdf text-danger me-2"></i>
                                        <span class="text-truncate"><?= htmlspecialchars($doc['title']) ?></span>
                                    </a>
                                <?php endforeach; ?>
                            <?php else: ?>
            ...
            <div class="col-md-8 d-flex flex-column">
                <div class="card shadow-sm flex-grow-1">
                    <div class="card-header d-flex justify-content-between align-items-center">Latest Announcements <i class="fas fa-bullhorn"></i></div>
                    <div class="card-body">
                        <div id="announcement-list" class="list-group list-group-flush" style="overflow-y: auto;">
                            <div class="text-center p-5"><div class="spinner-border" role="status"><span class="visually-hidden">Load...</span></div></div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <form action="upload_profile_picture.php" method="POST" enctype="multipart/form-data">
                    <div class="modal-header"><h5 class="modal-title">Upload New Profile Photo</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                    <div class="modal-body">
                        <p>Select an image file (JPG, PNG, GIF) with a maximum size of 2 MB.</p>
                        <div class="mb-3"><label for="profile_pic_input" class="form-label">Select File</label><input class="form-control" type="file" name="profile_pic" id="profile_pic_input" accept="image/png, image/jpeg, image/gif" required></div>
                    </div>
